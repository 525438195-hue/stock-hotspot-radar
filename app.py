...
def render_header(
    ...
    *,
    ...
    with st.container(border=True):
        ...
        cols = st.columns(3)
        cols[0].success("Tavily 已连接" if tavily_ready else "Tavily 未连接")
        cols[1].success("今日已刷新" if data_ready else "今日未刷新")
        cols[2].info("使用缓存" if _cache_status_text() == "是" else "实时刷新")
